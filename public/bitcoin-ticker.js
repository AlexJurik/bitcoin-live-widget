/**
 * Bitcoin Price Ticker Web Component
 * 
 * Usage:
 * 1. Include this script: <script src="bitcoin-ticker.js"></script>
 * 2. Add the component: <bitcoin-ticker></bitcoin-ticker>
 * 
 * Optional attributes:
 ...
 * 
 * Example:
 * <bitcoin-ticker refresh-interval="60" currency="USD" theme="dark"></bitcoin-ticker>
 */

class BitcoinTicker extends HTMLElement {
  constructor() {
    super();
  ...
  static get observedAttributes() {
    return ['refresh-interval', 'currency', 'theme'];
  }
  ...
  connectedCallback() {
    this.render();
    this.fetchPrice();
    this.intervalId = setInterval(() => this.fetchPrice(), this.refreshInterval);
  }
  ...
  async fetchPrice() {
    try {
      ...
      console.error('Bitcoin Ticker Error:', error);
      this.showError();
    }
  }
  ...
  getStyles() {
    ...
    return `
      ...
      .ticker {
        ...
        border: 1px solid ${isDark ? 'rgba(247, 147, 26, 0.3)' : 'rgba(247, 147, 26, 0.4)'};
        box-shadow: ${isDark 
          ? '0 4px 20px rgba(0, 0, 0, 0.4), 0 0 40px rgba(247, 147, 26, 0.1)' 
          : '0 4px 20px rgba(0, 0, 0, 0.1), 0 0 40px rgba(247, 147, 26, 0.05)'};
      ...
      .ticker:hover {
        ...
        box-shadow: ${isDark 
          ? '0 6px 24px rgba(0, 0, 0, 0.5), 0 0 50px rgba(247, 147, 26, 0.15)' 
          : '0 6px 24px rgba(0, 0, 0, 0.15), 0 0 50px rgba(247, 147, 26, 0.1)'};
      }
...
if (!customElements.get('bitcoin-ticker')) {
  customElements.define('bitcoin-ticker', BitcoinTicker);
}
